# -*- coding: utf-8 -*-
"""
Created on Tue Oct  9 20:10:06 2018
...
def Adam(alpha, beta_1, beta_2, epsilon, w_array, bias, input_array, epoch, valid):
    ...
    for i in range(epoch):
        ...
        graph_list.append(validation_loss)
...
for i in range(1,5):
    graph_list = []
    para_w, para_bias = Adam(i*para_alpha, para_beta_1, para_beta_2, para_epsilon, para_w_i, para_bias_i, training_array, para_epoch, validation_array)
    graph_list_final.append(graph_list)
    
import matplotlib.pyplot as plt

plt.plot(range(len(graph_list_final[0])),graph_list_final[0],'r-',label='alpha=0.001')
plt.plot(range(len(graph_list_final[1])),graph_list_final[1],'y-',label='alpha=0.002')
plt.plot(range(len(graph_list_final[2])),graph_list_final[2],'g-',label='alpha=0.003')
plt.plot(range(len(graph_list_final[3])),graph_list_final[3],'b-',label='alpha=0.004')
plt.ylabel('RMSE')
