# -*- coding: utf-8 -*-
"""
Created on Fri Oct 12 10:43:51 2018
...
def loss(w_array, bias, input_array):
    ...
    temp_w_array = w_array.reshape([1,162])
    return (total_loss/input_array.shape[0])**(0.5)  + para_lambda*0.5*np.dot(temp_w_array,temp_w_array.T)
...
def d_loss(w_array, bias, input_array):
    ...
    total_w += -2*temp_loss*input_array[0] + para_lambda * w_array
    total_b += -2*temp_loss
...
para_w_n = para_w.reshape([1,162])
L2Norm = np.sqrt(np.dot(para_w_n, para_w_n.T)[0][0])

print('L2Norm = %f' % L2Norm)
print('Training Loss = %f' % loss(para_w, para_bias, t_data_array))
...
with open('result_q3_4.csv', 'w') as f:
